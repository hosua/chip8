		...
		void execute(uint8_t op){
			...
			switch(op){
				...
				case Op::DRW:
				{
					...
					// The nibble is the height we will draw */
					// d_y is the y position of the line being drawn
					this->v[0xF] = 0;
					for (int d_y = 0; d_y < n; d_y++){
						px = this->mem[this->i + d_y];
						// d_x is the x position of the pixel in the line being drawn
						for (int d_x = 0; d_x < SPRITE_WIDTH; d_x++){
							if ((px & (0x80 >> d_x))){ // if pixel is set
								if(chip8.gfx[(x + d_x + ((y + d_y) * DISP_X))]){ // if gfx 
									v[0xF] = 1;	
								}
								// XOR Sprite onto screen
								chip8.gfx[x + d_x + ((y + d_y) * DISP_X)] ^= 1;
							}
						}
					}
